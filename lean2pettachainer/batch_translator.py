...
def essentialize_subgoal(goal_data: Any, *, kb: str = "kb", mode: str = "recurry", depth: int = 10) -> tuple[str, str]:
    ...
    if mode == "dynamic":
        ...
        for i, hyp in enumerate(hyps):
            ...
            cleanup_lines.append(f";; cleanup needed")
...
def run_demo(input_path: str, output_dir: str) -> None:
    # --- Read Goal(s) and Tactics from Input File ---
    ...
    # Normalize input to a list (so it supports both a single dict or an array of dicts)
    if isinstance(input_data, dict):
        if "goal" in input_data:
            input_data = [input_data]
        else:
            print("❌ Error: JSON format not recognized. Missing 'goal' key.")
            return
    ...
    # Iterate over every test in the JSON array
    for idx, test_case in enumerate(input_data):
        ...
        for i, g in enumerate(goals):
            ...
            script, cleanup = essentialize_subgoal(g, mode="dynamic")
            ...
            # (Optional) Add cleanup block if your essentialize_subgoal returns one
            if cleanup:
                 full_output_script.append(f"; --- Cleanup ---\n{cleanup}")

        if not goals:
             full_output_script.append("; No subgoals remain. Proof complete!")

        # --- File Writing Logic for this specific test ---
        safe_name = "".join(c for c in test_name if c.isalnum() or c in ('_', '-'))
        final_file_path = os.path.join(output_dir, f"{safe_name}.metta")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="Path to the input JSON file containing the 'goal' and 'tactics' (or an array of them)."
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="Path to the output directory where the .metta files will be saved (e.g., ./my_tests/)"
    )
